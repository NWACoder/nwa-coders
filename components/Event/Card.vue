<template>
	...
	class="relative border-8 border-white rounded-lg p-4 h-80 xl:h-96 w-auto cursor-pointer">
		<!-- day -->
		<div class="absolute -right-4 -top-4 bg-white text-black rounded-full p-4 h-12 w-12  flex items-center justify-center text-sm">
		...
		<div class="flex px-4 h-full">
			<div class="text-center text-lg xl:text-2xl self-center">
				{{item.properties.Name.title[0].plain_text}}
				<p v-if="item.properties.Speaker.rich_text.length > 0" class="text-sm pt-4">Guest Speaker<br>{{ item.properties.Speaker.rich_text[0].plain_text }}</p>
			</div>
